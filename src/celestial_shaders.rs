...
fn airwing_shader(_fragment: &Fragment, vertex: &Vertex, uniforms: &Uniforms) -> Color {
    ...
    let metallic = Color::from_float(0.72, 0.77, 0.82);
    let blue = Color::from_float(0.26, 0.72, 0.95);
    let shadow_blue = Color::from_float(0.05, 0.16, 0.32);
    let highlight = Color::from_float(0.80, 0.95, 1.0);

    let diff = normal.dot(&light_dir).max(0.0);
    let side_mask = ((vertex.position.x * 2.2).tanh() + 1.0) * 0.5; // separa mitad izquierda/derecha
    let depth_shade = ((vertex.position.z * 1.4).tanh() + 1.0) * 0.5;
    let base_side = mix_color(blue, metallic, side_mask);
    let diffuse = mix_color(shadow_blue, base_side, (0.35 + diff * 0.65).clamp(0.0, 1.0));
    let spec = normal.dot(&half_vector).max(0.0).powf(28.0);
    let specular = highlight * (spec * 0.85);
    ...
    let accent_wave = ((vertex.position.z * 8.0).sin() * 0.5 + 0.5) * 0.35;
    let top_glow = normal.y.max(0.0) * 0.25;
    let accent_weight =
        (depth_shade * 0.35 + accent_wave + hull_noise * 0.15 + top_glow).clamp(0.0, 0.75);
    let hull_color = mix_color(diffuse, metallic, accent_weight * side_mask)
        + mix_color(diffuse, blue, accent_weight * (1.0 - side_mask));
...
pub fn earth_like_shader(_fragment: &Fragment, vertex: &Vertex, uniforms: &Uniforms) -> Color {
    ...
    let is_land =
        (continent_base > land_threshold) || (continent_detail > 0.62 && continent_base > 0.42);
    ...
    let cloud_fine = fbm(pos.x * 12.0, pos.y * 12.0, pos.z * 12.0, 2) * 0.25;

    // Combinar capas (más nubes en zonas ecuatoriales)
    let latitude_cloud_factor = 1.0 - (pos.y.abs() * 0.5); // Más nubes cerca del ecuador
    let cloud_combined =
        (cloud_large * 0.5 + cloud_medium * 0.3 + cloud_fine) * latitude_cloud_factor;
...
pub fn saturn_like_shader(_fragment: &Fragment, vertex: &Vertex, uniforms: &Uniforms) -> Color {
    ...
    let ambient = 0.4;
    let spec = reflect(-light_dir, normal)
        .dot(&view_dir)
        .max(0.0)
        .powf(3.0)
        * 0.12;
...
pub fn ring_shader(_fragment: &Fragment, vertex: &Vertex, uniforms: &Uniforms) -> Color {
    ...
    let normalized_radius =
        ((radial_dist - INNER_RADIUS) / (OUTER_RADIUS - INNER_RADIUS)).clamp(0.0, 1.0);
...
pub fn ice_planet_shader(_fragment: &Fragment, vertex: &Vertex, uniforms: &Uniforms) -> Color {
    ...
    let aurora_pattern =
        ((pos.y * 8.0 + pos.x * 2.0) + fbm(pos.x * 4.0, pos.y * 4.0, pos.z * 4.0, 2) * 2.0).sin();
...
pub fn alien_planet_shader(_fragment: &Fragment, vertex: &Vertex, uniforms: &Uniforms) -> Color {
    ...
    base_color = mix_color(
        base_color,
        crystal_color,
        (crystal_pattern - 0.6).max(0.0) * 3.0,
    );
